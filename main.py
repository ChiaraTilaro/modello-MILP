...
# Tempi di processamento (Job 1-5)
P_raw = [[10, 5], [6, 7], [8, 4], [9, 6], [3, 11]]
    ...
    for f in fabbriche:
        ...
        for j in sequenza:
            ...
            for i in macchine:
                finito = c[j][i].x
                durata = P_raw[j-1][i]
                inizio = finito - durata
                # Formattiamo come [Inizio - Fine]
                riga += f"| [{inizio:>4.1f} - {finito:>4.1f}] (dur: {durata:>2}) "
            print(riga)
        print("\n")
